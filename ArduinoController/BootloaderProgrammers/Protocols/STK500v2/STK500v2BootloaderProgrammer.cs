...
using ArduinoController.BootloaderProgrammers.Protocols.STK500v2.Messages;
using ArduinoController.Hardware;
using ArduinoController.Hardware.Memory;

namespace ArduinoController.BootloaderProgrammers.Protocols.STK500v2 {
    internal class Stk500V2BootloaderProgrammer : ArduinoBootloaderProgrammer {
        ...
        protected override void Send(IRequest request) {
            int requestBodyLength  = request.Bytes.Length;
            ...
            byte checksum = 0;
            for (var i = 0; i < totalMessageLength - 1; i++) {
        ...
        public override void InitializeDevice() {
            uint hardwareVersion = GetParameterValue(Constants.ParamHwVer);
            uint softwareMajor   = GetParameterValue(Constants.ParamSwMajor);
            uint softwareMinor   = GetParameterValue(Constants.ParamSwMinor);
            uint vTarget         = GetParameterValue(Constants.ParamVTarget);
            Logger?.Info(
        ...
        public override void ExecuteWritePage(IMemory memory, int offset, byte[] bytes) {
            Logger?.Trace(
                "Sending execute write page request for offset "
              + $"{offset} ({bytes.Length} bytes)..."
            );

            byte writeCmd = _writeCommands[memory.Type];
            ...
            if (response == null || response.AnswerId != writeCmd
                ...
                throw new ArduinoUploaderException($"Executing write page request at offset {offset} failed!");
            }
        }

        public override byte[] ExecuteReadPage(IMemory memory) {
            byte readCmd = _readCommands[memory.Type];
